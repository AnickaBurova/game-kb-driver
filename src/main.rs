...
fn main() {
    ...
    ////{
        ////let mut new_devices = discover_devices(&mut context, &discovered, &mappings).unwrap();
        ////for (address, device) in new_devices.drain(..) {
            ////match device {
                ////Ok((handle, device_map)) => {
                    ////discovered.add_device(address, handle, device_map);
                ////}
                ////Err(err) => println!("Error getting device at bus: {} address {}", address >> 8, address & 0xff),
            ////}
        ////}
    ////}
    //let mut handle = None;
